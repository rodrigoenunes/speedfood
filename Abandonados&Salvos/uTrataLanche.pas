...
begin
  ...
  with FuTrataLanche
  do begin
    LabNomeLanche.Caption := uDM.ItensDescricao.AsString;
...
begin
  ...
  if (nCol <> 1) and (nCol <> 3) and (nCol <> 6) and (nCol <> 8) then Exit;
  //
  ...
  if (nCol = 1) or (nCol = 6) then lSoma := True
                              else lSoma := False;
